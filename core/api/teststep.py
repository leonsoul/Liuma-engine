...
class ApiTestStep:
    ...
    def execute(self):
        ...
        try:
            ...
            request_log = "【请求信息】:<br>"
            request_log += "{} {}<br>".format(
                self.collector.method, url_join(self.collector.url, self.collector.path)
            )
                ...
                if value is not None:
                    ...
                    if key == "files":
                        ...
                        if isinstance(value, list):
                            request_log += "{}: {}<br>".format(
                                c_key, [i[1][0] for i in value]
                            )
                    elif c_key == "请求体":
                        request_log += "<span>{}: {}</span><br>".format(
                            c_key, dict2str(value)
                        )
                    else:
            ...
            if (
                self.collector.body_type in ("text", "xml", "html")
                and "data" in self.collector.others
            ):
                self.collector.others["data"] = str(
                    self.collector.others["data"]
                ).encode("utf-8")
            if (
                "files" in self.collector.others
                and self.collector.others["files"] is not None
            ):
            ...
            if self.collector.controller["encryption"].lower() == "setting":
                ...
                if self.collector.others["params"] is not None:
                    ...
                    url += "/" + Signature().concatenating_url(
                        self.collector.others["params"]
                    )
                ...
                # 只是简单的加密
                signature_string, signature = Signature().sign_url_v4(
                    self.collector.controller["token"],
                    args_map,
                    no_sign_date=self.collector.private["no_sign_data"],
                ...
                if self.collector.others["params"] is not None:
                    args_map.update(self.collector.others["params"])
                    # 对url进行加密
                    url = self.collector.url + Signature().sign_url_v4c(
                        self.collector.path, args_map
                    )
            # 前置等待
            if int(self.collector.controller["sleepBeforeRun"]) > 0:
                sleep(int(self.collector.controller["sleepBeforeRun"]))
                self.test.debugLog(
                    "请求前等待%sS" % int(self.collector.controller["sleepBeforeRun"])
                )
            start_time = datetime.datetime.now()
            # 判断是否使用session
            if (
                self.collector.controller["useSession"].lower() == "true"
                and self.collector.controller["saveSession"].lower() == "true"
            ):
                res = self.session.session.request(
                    self.collector.method, url, **self.collector.others
                )
            ...
            if self.collector.others.get("data"):
                DebugLogger(
                    "开始请求，请求参数为{}，请求路径为{}".format(
                        json.dumps(self.collector.others["data"], ensure_ascii=True),
                        url,
                    )
                )
            ...
            # 记录结束时间、保存响应结果
            self.test.recordTransDuring(
                int((end_time - start_time).microseconds / 1000)
            )
            ...
            if "content-disposition" not in [
                key.lower() for key in self.response_headers.keys()
            ]:
                response_text = "<b>响应体: {}</b>".format(log_msg(self.response_content))
            else:
                response_text = "<b>响应体: 文件内容暂不展示, 长度{}</b>".format(
                    len(self.response_content_bytes)
                )
            # 响应体长度不能超过50000
            ...
            if int(self.collector.controller["sleepAfterRun"]) > 0:
                ...
                self.test.debugLog(
                    "请求后等待%sS" % int(self.collector.controller["sleepAfterRun"])
                )
    ...
    def looper_controller(self, case, api_list, step_n):
        ...
        if "type" in self.collector.looper and self.collector.looper["type"] == "WHILE":
            ...
            while (
                self.collector.looper["timeout"] == 0
                or (datetime.datetime.now() - loop_start_time).seconds * 1000
                < self.collector.looper["timeout"]
            ):  # timeout为0时可能会死循环 慎重选择
                # 渲染循环控制控制器 每次循环都需要渲染
                _looper = case.render_looper(self.collector.looper)
                result, _ = LMAssert(
                    _looper["assertion"], _looper["target"], _looper["expect"]
                ).compare()
                if not result:
                    break
            ...
            for index in range(_looper["times"]):  # 本次循环次数
                self.context[
                    _looper["indexName"]
                ] = index  # 给循环索引赋值第几次循环 母循环和子循环的索引名不应一样
                _api_list = api_list[step_n : (step_n + _looper["num"])]
                case.loop_execute(_api_list, api_list[step_n]["apiId"])
    ...
    def condition_controller(self, case):
        ...
        for condition in _conditions:
            try:
                result, msg = LMAssert(
                    condition["assertion"], condition["target"], condition["expect"]
                ).compare()
                if not result:
                    return msg
    ...
    def exec_sql(self, sql, case):
        ...
        if sql["sqlType"] != "query":
            ...
            names = [
                value.strip() for value in re.split("[,，]", sql["names"])
            ]  # name数量可以比结果数量段，但不能长，不能会indexError
    ...
    def check(self):
        ...
        if self.collector.assertions is not None:
            ...
            for items in self.collector.assertions:
                try:
                    if items["from"] == "resCode":
                        ...
                        actual = extract(
                            items["method"], self.response_headers, items["expression"]
                        )
                    elif items["from"] == "resBody":
                        actual = extract(
                            items["method"], self.response_content, items["expression"]
                        )
                    else:
                        raise ExtractValueError("无法在{}位置进行断言".format(items["from"]))
                    result, msg = LMAssert(
                        items["assertion"], actual, items["expect"]
                    ).compare()
            ...
            final_result, msg = LMAssert(
                "相等", str(self.status_code), str(200)
            ).compare()
            check_messages.append(msg)
        self.assert_result = {
...
if __name__ == "__main__":
    ...
    print(
        oss_util(
            d["AccessKeyId"], d["AccessKeySecret"], d["SecurityToken"]
