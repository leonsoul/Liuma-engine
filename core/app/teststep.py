...
class AppTestStep:
    ...
    def looper_controller(self, case, opt_list, step_n):
        ...
        if self.collector.opt_trans == "While循环":
            ...
            while (
                timeout == 0
                or (datetime.now() - loop_start_time).seconds * 1000 < timeout
            ):
                # timeout为0时可能会死循环 慎重选择
                self.context[index_name] = index  # 给循环索引赋值第几次循环 母循环和子循环的索引名不应一样
                _looper = case.render_looper(
                    self.collector.opt_data
                )  # 渲染循环控制控制器 每次循环都需要渲染
                index += 1
                result, _ = LMAssert(
                ...
                if not result:
                    break
                _opt_list = opt_list[
                    step_n + 1 : (step_n + _looper["steps"] + 1)
                ]  # 循环操作本身不参与循环 不然死循环
    ...
    def assert_controller(self):
        if self.collector.opt_type == "assertion":
            if self.result[0]:
                self.test.debugLog(
                    "[{}]断言成功: {}".format(self.collector.opt_trans, self.result[1])
                )
            else:
                self.test.errorLog(
                    "[{}]断言失败: {}".format(self.collector.opt_trans, self.result[1])
                )
                self.test.saveScreenShot(
                    self.collector.opt_trans, self.device.screenshot(format="raw")
                )
                if (
                    "continue" in self.collector.opt_data
                    and self.collector.opt_data["continue"] is True
                ):
                    try:
    ...
    def condition_controller(self, current):
        if self.collector.opt_type == "condition":
            ...
            if self.result[0]:
                self.test.debugLog(
                    "[{}]判断成功, 执行成功分支: {}".format(
                        self.collector.opt_name, self.result[1]
                    )
                )
                return [
                    current + i
                    for i in range(offset_true + 1, offset_true + offset_false + 1)
                ]
            else:
                self.test.errorLog(
                    "[{}]判断失败, 执行失败分支: {}".format(
