...
class Signature:
    ...
    def open_sign_url(appKey, secretKey, args_map):
        ...
        timestamp = str(
            int(time.time() * 1000)
        )  # 请求发起时间戳        # timestamp ='1627525820470'  # 请求发起时间戳
        std_args_map = {
            "timestamp": timestamp,
            "appKey": appKey,
            "secretKey": secretKey,
    ...
    # sign_url_v4 返回V4签名
    def sign_url_v4(
        self, token, args_map, source=PC_source, limit=False, no_sign_date=None
    ):
        """
        sign_url_v4 返回V4签名
        @param token: 用户token
    ...
    def sign_url_v4c(path, args_map):
        ...
        hl = hashlib.md5()
        hl.update(
            (alltuu_config.CDNKey + "/" + file_path + time_stamp).encode(
                encoding="utf-8"
            )
        )
        return "/{sign}/{time_stamp}/{file}".format(
            sign=hl.hexdigest(), time_stamp=time_stamp, file=file_path
        )
